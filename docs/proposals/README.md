...
The tier-1 feature bundle (`materialize_query`, `find_outliers`, `power_analysis`, `regression_line` + `residual_diagnostic`) shipped and was folded into `docs/SPEC.md` §5 (entries 5.5a, 5.6a, 5.10a, 5.12a, 5.12b); `pairwise_comparisons` shipped next and was folded into §5.9a (the post-hoc follow-up to `compare_groups`, growing the tool surface 21 → 22). Per the convention above, the proposal files themselves were deleted once folded; the git history retains the design context. The prefix-replay-guards proposal (2026-07-18) shipped in 1.5.0 and was folded into SPEC §5.1 / §5.11 / §5.11d / §6.
